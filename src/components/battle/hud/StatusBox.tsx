...
function Gauge({
  ...
  return (
    <div className={cn("flex items-center gap-1.5", className)}>
      <span
        aria-hidden
        className="rounded-[3px] bg-gradient-to-b from-[#ffd257] to-[#e0961b] px-1 py-px font-display text-[9px] leading-none font-black text-[#2a1a02] italic"
      >
        {label}
      </span>
      <div
        ...
        className={cn(
          "relative h-2 flex-1 overflow-hidden rounded-full border border-black/70 bg-[#0b1220]",
...
export function StatusBox({
  ...
  return (
    <div
      ...
      className="relative w-[17rem] max-w-[47vw] drop-shadow-[0_6px_14px_rgba(0,0,0,0.55)]"
    >
      ...
      >
        <defs>
          <linearGradient id={`sb-${side}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#22354f" stopOpacity="0.96" />
            <stop offset="55%" stopColor="#121e30" stopOpacity="0.96" />
            <stop offset="100%" stopColor="#0a1220" stopOpacity="0.96" />
          </linearGradient>
        </defs>
        <path
      ...
      <div
        className={cn(
          "relative px-4 py-2",
          isPlayer ? "pr-5 pl-4" : "pr-4 pl-5",
        )}
      >
        <div className="flex items-baseline gap-1.5">
          <p
            className="min-w-0 flex-1 truncate text-[13px] font-bold tracking-wide text-white"
          ...
          <p
            className="shrink-0 text-[12px] font-bold text-white"
            style={{ textShadow: "0 1px 2px rgba(0,0,0,0.9)" }}
          >
            <span className="mr-0.5 text-[9px] text-[#b9c7dc] italic">
              {t.lvShort}
            </span>
            {battler.level}
          </p>
        ...
        <div className="mt-1 flex items-center justify-between gap-2">
          ...
          {isPlayer && (
            <p
              className="text-[11px] font-bold text-white tabular-nums"
              style={{ textShadow: "0 1px 2px rgba(0,0,0,0.9)" }}
            >
              {Math.max(0, battler.hp)}
              <span className="text-[#b9c7dc]">/{battler.maxHp}</span>
            </p>
          )}
        </div>
        ...
        {isPlayer && (
          <div className="mt-1 flex items-center gap-1.5">
            <span
              aria-hidden
              className="text-[8px] font-black tracking-widest text-[#7fd8ff] italic"
            >
              EXP
            </span>
            <div className="h-[3px] flex-1 overflow-hidden rounded-full bg-[#0b1220] shadow-[inset_0_1px_1px_rgba(0,0,0,0.9)]">
              <div
        ...
        {isPlayer && battler.ability && (
          <p
            className="mt-0.5 truncate text-[9px] font-semibold text-[#b9c7dc]"
            style={{ textShadow: "0 1px 2px rgba(0,0,0,0.9)" }}
          >
