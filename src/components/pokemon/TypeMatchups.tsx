...
export function TypeMatchups({ matchups }: TypeMatchupsProps) {
  ...
  if (rows.length === 0) {
    return <p className="font-mono text-xs text-slate-400">Sin datos de tipos.</p>;
  }
  ...
  return (
    <div className="relative flex flex-col">
      ...
      {rows.map((row) => (
        ...
        >
          <span
            className={`flex size-11 items-center justify-center rounded-full border bg-[#070b14] font-mono text-sm font-bold ${row.node}`}
          >
            {row.factor}
          </span>
          <div>
