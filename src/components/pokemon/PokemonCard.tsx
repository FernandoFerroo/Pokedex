...
export function PokemonCard({ entry }: PokemonCardProps) {
  ...
  return (
    <Link
      ...
      className="aura-card group relative flex flex-col overflow-hidden rounded-lg border bg-gradient-to-b from-hud-1 to-hud-3 p-2 motion-safe:hover:-translate-y-0.5 [content-visibility:auto] [contain-intrinsic-size:auto_120px] max-sm:p-1.5 sm:p-3 sm:[contain-intrinsic-size:auto_230px]"
    >
      ...
      <div className="flex items-center justify-between text-slate-400">
        <span className="font-pixel text-[10px] transition-colors group-hover:text-[var(--aura)] max-sm:text-[9px] sm:text-xs">
      ...
      {/* Favorito y alta directa al equipo, sin pasar por la ficha:
          corazón arriba, «+» debajo. On touch there is no hover to reveal
          them with, so on phones they stay visible. */}
      <span className="absolute top-7 right-1 z-20 flex flex-col gap-1 opacity-80 transition-opacity duration-200 group-hover:opacity-100 focus-within:opacity-100 max-sm:top-5 max-sm:right-0.5 max-sm:gap-0.5 sm:top-9 sm:right-2 sm:gap-1.5 sm:opacity-0">
      ...
      {/* En móvil la altura del sprite manda sobre el ancho de la columna: una
          caja fija de 64px deja la ficha en ~120px de alto, lo justo para que
          entren tres filas y media bajo los filtros. */}
      <div className="aura-halo relative mx-auto aspect-square w-full max-w-[130px] max-sm:aspect-auto max-sm:h-16">
        <Image
      ...
      <p className="truncate text-sm font-semibold tracking-tight text-slate-100 transition-colors group-hover:text-slate-50 max-sm:text-[11px] max-sm:leading-tight sm:text-base">
